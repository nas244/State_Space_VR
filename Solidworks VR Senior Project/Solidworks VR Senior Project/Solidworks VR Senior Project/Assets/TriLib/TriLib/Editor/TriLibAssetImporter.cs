    ...
    public static class TriLibAssetImporter
    {
        public static void Import(string assetPath)
        {
            ...
            using (var assetLoader = new AssetLoader())
            {
                ...
                assetLoader.AnimatorControllerCreation += () => AnimatorController.CreateAnimatorControllerAtPath(assetPath + ".controller");//new AnimatorController { name = assetImporter.name };
        ...
        private static void AddSubAsset(Object asset, string prefabPath)
        {
            ...
            foreach (var subAsset in subAssets)
            {
                if (subAsset.name == asset.name && asset.GetType() == subAsset.GetType())
                {
                    ...
                    Debug.LogWarning("["+ asset.GetType().Name + "]" + asset.name + "(" + asset.GetInstanceID() + ") already exists in " + prefabPath);
#endif    
					if (subAsset == asset)
                    {
                        return;

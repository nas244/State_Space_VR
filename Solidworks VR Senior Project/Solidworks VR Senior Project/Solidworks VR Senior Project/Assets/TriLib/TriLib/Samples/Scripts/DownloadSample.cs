    ...
    namespace Samples
    {
        public class DownloadSample : MonoBehaviour
        {
            ...
            private void Start()
            {
                ...
                _fileDownloaders = new WWW[urls.Length];
            }
            ...
            {
                _fileDownloaders[index] = new WWW(url);
                yield return _fileDownloaders[index];
                if (fileExtension == ".zip")
                {
#if TRILIB_USE_ZIP
                using (var memoryStream = new MemoryStream(_fileDownloaders[index].bytes))
                {
                ...
                File.WriteAllBytes(localFilename, _fileDownloaders[index].bytes);
                LoadFile(fileExtension, localFilename);
